...
function summarizeItemCollection(items) {
  ...
  return items
    .map((item) => {
      ...
      const title = firstFilled(item.title, item.name, item.product, item.sku, item.offer, item.item);
      const quantity = parseDecimal(firstFilled(item.quantity, item.qty, item.count, item.amount));
      const price = parseDecimal(firstFilled(item.price, item.cost, item.sum, item.total));
      const parts = [title || "товар"];
      if (quantity) {
...
function normalizeWebhookPayload(value) {
  if (Array.isArray(value)) {
    return value.map((item) => normalizeWebhookPayload(maybeParseJson(item)));
  }

  if (value && typeof value === "object") {
    const result = {};
    for (const [key, nestedValue] of Object.entries(value)) {
      result[key] = normalizeWebhookPayload(maybeParseJson(nestedValue));
    }
    return result;
  }
...
function flattenPayloadEntries(value, prefix = "", entries = []) {
  if (Array.isArray(value)) {
    if (value.every((item) => item && typeof item === "object")) {
      entries.push([prefix, summarizeItemCollection(value)]);
    } else {
      entries.push([prefix, value.map((item) => normalizeLooseText(item)).filter(Boolean).join(", ")]);
    }
    value.forEach((item, index) => {
      flattenPayloadEntries(item, prefix ? `${prefix}[${index}]` : String(index), entries);
    });
    return entries;
  }
  ...
  if (value && typeof value === "object") {
    for (const [key, nestedValue] of Object.entries(value)) {
      ...
      flattenPayloadEntries(nestedValue, nextPrefix, entries);
    }
    return entries;
  }
...
function pickField(payload, patterns) {
  ...
  const entries = flattenPayloadEntries(normalizedPayload);
...
function inferOrderFromPayload(payload) {
  ...
  const address = firstFilled(
    pickField(normalizedPayload, [/адрес/, /address/, /street/, /улиц/]),
    pickField(normalizedPayload, [/delivery.*address/, /shipping.*address/])
  );
  const customerFields = splitFullName(
    firstFilled(
      pickField(normalizedPayload, [/фио/, /получател/, /заказчик/, /customer/, /client/, /^name$/, /full.?name/]),
      pickField(normalizedPayload, [/first.?name/, /^name$/]),
      pickField(normalizedPayload, [/last.?name/, /surname/, /family/])
    )
  );
  const phone = firstFilled(pickField(normalizedPayload, [/телефон/, /phone/, /mobile/]));
  const phoneNormalized = normalizePhone(phone);
  const itemsSummary = firstFilled(
    pickField(normalizedPayload, [/товар/, /items?$/, /products?$/, /basket/, /cart/]),
    pickField(normalizedPayload, [/items?\./, /products?\./])
  );
  const items = parseItems(itemsSummary);
  const orderId = normalizeOrderId(
    firstFilled(
      pickField(normalizedPayload, [/^id$/, /order.?id/, /lead.?id/, /request/, /transaction/, /tranid/, /payment.?id/, /formid/])
    )
  );
  const deliveryPriceRaw = firstFilled(pickField(normalizedPayload, [/delivery.*price/, /стоим.*достав/]));
  const totalPriceRaw = firstFilled(pickField(normalizedPayload, [/^amount$/, /^sum$/, /total/, /сумм/]));
  const currency = inferCurrency(totalPriceRaw, deliveryPriceRaw, pickField(normalizedPayload, [/currency/, /валют/])) ||
    (totalPriceRaw || deliveryPriceRaw ? "RUB" : "");

  return {
    id: orderId || crypto.randomBytes(9).toString("base64url"),
    source: "tilda-webhook",
    createdAt: parseDate(firstFilled(pickField(normalizedPayload, [/created/, /date/]), new Date().toISOString())) || new Date().toISOString(),
    updatedAt: parseDate(firstFilled(pickField(normalizedPayload, [/updated/]), new Date().toISOString())) || new Date().toISOString(),
    customerName: customerFields.customerName,
    firstName: customerFields.firstName,
    lastName: customerFields.lastName,
    phone,
    phoneNormalized,
    email: firstFilled(pickField(normalizedPayload, [/email/, /e-mail/, /почт/])),
    address,
    city: firstFilled(pickField(normalizedPayload, [/город/, /city/])),
    postalCode: firstFilled(pickField(normalizedPayload, [/индекс/, /zip/, /postal/])),
    country: firstFilled(pickField(normalizedPayload, [/country/, /страна/])),
    trackNumber: normalizeTrackNumber(firstFilled(pickField(normalizedPayload, [/трек/, /track/]))),
    orderStatus: firstFilled(pickField(normalizedPayload, [/order.*status/, /^status$/, /статус/])),
    deliveryStatus: firstFilled(pickField(normalizedPayload, [/delivery.*status/, /статус.*достав/])),
    paymentStatus: firstFilled(pickField(normalizedPayload, [/payment.*status/, /статус.*оплат/])),
    paymentMethod: firstFilled(pickField(normalizedPayload, [/payment.*method/, /способ.*оплат/, /paymentsystem/])),
    deliveryMethod: firstFilled(pickField(normalizedPayload, [/delivery.*method/, /^delivery$/, /доставк/])),
    deliveryMethodName: firstFilled(pickField(normalizedPayload, [/delivery.*name/, /^delivery$/, /назван.*достав/])),
    deliveryPrice: parseDecimal(deliveryPriceRaw),
    deliveryDate: firstFilled(pickField(normalizedPayload, [/delivery.*date/, /дата.*достав/])),
    totalPrice: parseDecimal(totalPriceRaw),
    currency,
    items,
    itemsSummary,
    flags: normalizeFlags([
      pickField(normalizedPayload, [/source/, /formname/]),
      pickField(normalizedPayload, [/payment.*system/, /paymentsystem/])
    ]),
    notes: firstFilled(pickField(normalizedPayload, [/коммент/, /comment/, /note/, /примеч/])),
    raw: {
      source: "tilda-webhook",
...
function extractOrderLookupCriteria(text) {
  ...
  const hasPhoneIntent = /(?:телефон\w*|номер\w*|phone|call|мой номер|мой телефон|у меня номер|посмотри по номеру|найди по номеру)/i.test(rawText);
  const hasTrackIntent = /(?:трек\w*|track|отслед|посылк)/i.test(rawText);
  const phoneFromContext =
    rawText.match(/(?:телефон\w*|номер\w*|phone|мой номер|мой телефон|у меня номер|посмотри по номеру|найди по номеру)\D{0,20}(\+?\d[\d\s().-]{8,}\d)/i)?.[1] || "";
  ...
  const trackNumber =
    ...
    (!phoneFromContext && hasTrackIntent && !hasPhoneIntent
...
function isOrderQuestion(text) {
  return /(заказ|доставк|трек|track|посылк|отправлен|статус|где мой|когда достав|отслед|crm|оплат|телефон|номер|номеру телефона|получател|адрес|мой телефон|мой номер|у меня номер|посмотри по номеру|найди по номеру|посмотри по|найди по)/i.test(text);
}
...
function formatOrderSummaryLine(order) {
  const computed = deriveComputedOrderStatus(order);
  const parts = [];

  if (order.createdAt) {
    parts.push(`дата ${formatOrderDate(order.createdAt)}`);
  }
  if (computed.finalStatus) {
    parts.push(computed.finalStatus);
  }
  if (order.trackNumber) {
    parts.push(`трек ${order.trackNumber}`);
  }
  if (order.totalPrice !== null && order.totalPrice !== undefined) {
    parts.push(`сумма ${formatMoney(order.totalPrice, order.currency)}`);
  }
  if (order.itemsSummary) {
    const compactItems = normalizeLooseText(order.itemsSummary);
    parts.push(`заказ ${compactItems.length > 90 ? `${compactItems.slice(0, 87)}...` : compactItems}`);
  }

  const prefix = order.customerName || `Заказ ${order.id}`;
...
function formatOrderReply(matches, criteria) {
  ...
  if (matches.length > 1) {
    const lines = [`Нашлось ${matches.length} заказов.`];
    for (const order of matches.slice(0, 5)) {
      lines.push(`- ${formatOrderSummaryLine(order)}`);
    }
    if (matches.length > 5) {
      lines.push(`И ещё ${matches.length - 5} заказов по этим данным.`);
    }
    return lines.join("\n");
  }
  ...
  lines.push(computed.isCompleted ? "Заказ завершён." : "Заказ ещё не завершён.");
  lines.push(computed.isDelivered ? "Доставка подтверждена." : "Подтверждения доставки пока нет.");
