...
function extractOrderLookupCriteria(text) {
  ...
  const asksForPhone = /(телефон|phone|позвон|номер телефона)/i.test(rawText);
  const asksForTrack = /(трек|track|отслед|посылк)/i.test(rawText);
  const possibleTrackTokens = rawText.match(/[A-ZА-Я0-9-]{6,}/gi) || [];
  const trackNumber = asksForTrack
    ? possibleTrackTokens
        .map((item) => normalizeTrackNumber(item))
        .find((item) => item)
    : "";
  const phoneMatch = !asksForTrack || asksForPhone ? rawText.match(/(?:\+?\d[\d\s().-]{8,}\d)/) : null;
...
function formatOrderReply(matches, criteria) {
  ...
  if (order.deliveryDate) {
    lines.push(`Дата доставки: ${order.deliveryDate}`);
  }
