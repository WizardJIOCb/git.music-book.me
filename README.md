...
Рекомендация для Tilda:

1. Включи отправку сразу после оформления заказа, а не только периодическую выгрузку.
2. Если доступен выбор формата, используй стандартный webhook Tilda или `application/x-www-form-urlencoded`.
3. Если в Tilda есть тестовая отправка webhook, проверь, что сервер отвечает `200 OK`.
...
Что webhook умеет разбирать сейчас:

- обычные плоские поля Tilda
- form-urlencoded поля вида `order[id]`, `payment[status]`, `delivery[address]`
- JSON-строки внутри отдельных полей, если Tilda передаёт вложенные объекты или массивы товаров
- данные о заказе, оплате, доставке и товарах в одном payload
